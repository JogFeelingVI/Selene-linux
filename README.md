...
![Selene Linux 运行截图](snapshot.png)
...
- **原生硬件解码**：原生链接 Linux 本地的 `libmpv.so`，支持调用系统的 **VA-API / NVDEC** 硬件解码，显卡直接参与视频渲染，降低 CPU 占用与温度。
...
为了方便 Linux 用户监控显卡硬解状态与查看系统时间，本项目内置了一个**支持全面交互的全局硬件统计悬浮窗（HUD）**：

*   **多档模式切换快捷键**：在视频播放界面中（**窗口模式或全屏模式下均可**），按下键盘上的 **`Left-Shift + Backspace`**（左侧 Shift + 退格键）可循环切换显示状态：
    *   **模式 1 (极简模式)**：仅以精简横条显示视频物理分辨率、播放状态及当前进度，最大程度减少画面干扰。
    *   **模式 2 (详细面板)**：显示全面的系统状态、媒体详情、缓冲情况、音轨字幕轨，并集成倍速调节及播放控制组件。
    *   **关闭模式**：隐藏 HUD 悬浮窗。
*   **拖拽与悬停反馈**：
    *   **自由拖拽**：在悬浮窗开启状态下，使用鼠标按住面板的**任意位置均可随心拖动**它到屏幕任意处，防止遮挡画面。
    *   **智能悬停**：鼠标滑出面板时，它会自动降低不透明度（**75% 变暗**），鼠标划入时自动恢复不透明（**100% 变亮**），既保证视线通透，又兼顾交互灵敏。
*   **一键倍速调整 (1.0x / 1.5x / 2.0x)**：
    *   **界面点击**：在模式 2（详细面板）下直接集成了 `1.0x`、`1.5x`、`2.0x` 调节按钮，点击即可实时变速。
    *   **全局快捷键**：
        *   按 **`Left-Shift + 2`**：一键快速切换至 **2.0 倍速** 播放。
        *   按 **`Left-Shift + 1`**：一键快速恢复至 **1.0 倍速** 正常播放。
*   **悬浮窗展示的信息**：
    ...
    *   **网络缓冲监控**：若处于缓冲中，会高亮显示 `加载中...`；正常播放时则显示当前的 `已缓冲时长`（如 `00:03:12`），便于网络视频排障。
    *   **音轨与字幕轨状态**：实时读取底层播放器当前正启用的音轨语言/名称和字幕轨道。
    *   **渲染模式**：实时展示显卡硬件加速状态（EGL Context - 独占通道）。
    *   **内置播放控制**：面板上集成了点击式播放/暂停按键，以及直接关闭悬浮窗的叉号。
...
由于本目录以及工作流文件属于本 Fork 项目专属，**在与官方上游（Upstream）同步时，通常不会产生任何 Git 代码合并冲突**。
...
- **解决方案**：
  - **方法 A：退回到 X11 兼容模式运行**（通常兼容性更好，能够正常恢复光标和动画）：
    ```bash
